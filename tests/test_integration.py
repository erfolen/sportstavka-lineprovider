...
@pytest.mark.asyncio
async def test_get_event(client, session):
    """Тест получение события"""
    async for cl in client:
        response = await cl.get("/event", params={"event_id": "event_247"})
        print('test-pr', response)
        #Проверяем, успешный ответ
        assert response.status_code == 200
        response_data = response.json()
        assert response_data["event_id"] == "event_247"
        assert response_data["coefficient"] == 3.3
        assert response_data["deadline"] == "2024-09-16T18:57:00"
        assert response_data["state"] == "new"
